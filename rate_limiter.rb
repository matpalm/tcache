...
class RateLimiter
	...
	def checkpoint
		...
		if @allowed > 0
			@allowed -=1
#			puts "RateLimiter: allowed now #{@allowed}"
		else
...
#		puts "RateLimiter: recheck_with_twitter"
		while true
			begin
...
#				puts "RateLimiter: problem checking rate limit?? #{Time.now}"
				sleep 60
			end
		end
	end
...
#			puts "RateLimiter: sleeping #{minutes_till_reset} minutes until #{@reset_time}"
